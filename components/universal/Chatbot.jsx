            ...
            <div className="bg-white rounded-lg p-6 w-[450px] h-[600px] flex flex-col">
                ...
                <div className="flex">
                    ...
                    <button 
                        onClick={sendMessage} 
                        disabled={isLoading} 
                        className={`px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:bg-blue-600 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                        <FaPaperPlane />
                    </button>
                </div>
                <button onClick={onClose} className="mt-4 px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 focus:outline-none focus:bg-gray-400">
                    Close
                </button>
            </div>
        </div>
...
export default function ChatBot() {
    ...
    return (
        <div>
            <button className='fixed bottom-16 right-20 text-white bg-black p-4 rounded-full' onClick={toggleMessagingModal}>
                <FaMessage size={28} className='' />
            </button>
